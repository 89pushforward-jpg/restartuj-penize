  ...
  return (
    <main className="min-h-screen overflow-hidden bg-background text-foreground">
      ...
      <section id="top" className="relative isolate bg-brand-navy text-white">
        ...
        <div className="mx-auto grid max-w-7xl gap-14 px-5 py-16 lg:grid-cols-[1fr_1fr] lg:items-center lg:px-8 lg:py-22">
          <div>
            ...
            <p className="mt-4 text-xs leading-5 text-slate-400">13 PDF • jednorázová platba • bez předplatného • bez následného prodejního hovoru</p>
          </div>
      ...
      <section className="bg-white px-5 py-20 lg:px-8 lg:py-24">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="map-panel">
            ...
            <div className="map-panel-copy"><div className="flex items-center gap-3"><img src="/brand/mark-official.png" width="960" height="960" alt="" className="size-14 object-contain" /><span className="inline-flex rounded-full bg-brand-green/12 px-3 py-1.5 text-xs font-extrabold uppercase tracking-[.12em] text-brand-green">Mapa ven</span></div><h3>Nejdřív ochránit peníze. Potom pochopit celek a řešit konkrétní situaci.</h3><p>Finanční štít ukáže časté podvody. Finanční (Re)Start vytvoří základ, Slovník investora pomůže s pojmy a šest praktických průvodců otevřete právě tehdy, když řešíte konkrétní rozhodnutí.</p><div className="mt-6 flex flex-wrap gap-2 text-xs font-bold text-slate-200"><span>1. Ochraňovat</span><ArrowRight /><span>2. Zorientovat se</span><ArrowRight /><span>3. Rozhodnout se bez tlaku</span></div></div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-white px-5 py-22 lg:px-8">
        <div className="mx-auto max-w-7xl"><div className="grid gap-12 lg:grid-cols-[.8fr_1.2fr] lg:items-start"><div><p className="eyebrow">Zpětná vazba prvních čtenářů</p><h2 className="section-title">Oceňují strukturu, příklady a čtivost.</h2><div className="mt-5 flex flex-wrap items-center gap-x-3 gap-y-1" aria-label="Hodnocení prvních čtenářů 4,7 z 5"><span className="rating-stars text-xl" aria-hidden="true" /><strong className="text-lg text-brand-navy">4,7 / 5</strong><span className="text-sm text-slate-500">hodnocení prvních čtenářů</span></div><p className="section-lead">Konkrétní dojmy prvních lidí, kteří dostali materiály k přečtení — bez anonymních citací a bez přikrášlování.</p><figure className="section-illustration mt-8"><img src="/story/readers-learning-cz-young.png" width="1536" height="1024" alt="Ilustrace mladých českých čtenářů, kteří používají finanční příručky v praxi" /><figcaption>Jeden systém, různé situace a vlastní tempo čtení.</figcaption></figure></div><div className="grid gap-4 sm:grid-cols-2">{testimonials.map((quote, index) => <figure key={quote.name} className={cn('rounded-3xl border border-slate-200 bg-[#f8faf8] p-7', index === 2 && 'sm:col-span-2')}><span className="rating-stars mb-4 text-sm" aria-hidden="true" /><blockquote className="text-lg leading-8 text-brand-navy">„{quote.text}“</blockquote><figcaption className="mt-6 flex items-center gap-3 text-sm font-bold text-slate-700"><span className="grid size-9 place-items-center rounded-full bg-brand-navy text-brand-green">{quote.name.charAt(0)}</span>{quote.name}</figcaption></figure>)}</div></div></div>
      </section>
      ...
      <section id="o-projektu" className="bg-[#f5f8f6] px-5 py-22 lg:px-8">
        <details className="founder-card mx-auto max-w-6xl">
          <summary>
            ...
            <div><p className="eyebrow">Kdo za projektem stojí</p><h2>Lukáš Zahrádka</h2><p>Nejsem finanční poradce ani zástupce banky. Jsem podnikatel, který se musel z finančního zmatku vymotat sám — a díky tomu začal rozumět investování, promyšleněji zhodnocovat vlastní peníze a včas poznávat nabídky, kterým je lepší se vyhnout.</p><div className="founder-tags"><span><Check /> podnikatelská zkušenost</span><span><Check /> z chaosu k vlastnímu systému</span><span><Check /> žádný prodej produktů</span></div></div>
            <span className="founder-open-label">Poznat můj příběh <ChevronDown /></span>
          </summary>
          <div className="founder-card-detail">
            <div><p className="book-kicker">Proč Restartuj Peníze vzniklo</p><h3>Nejdřív jsem potřeboval napravit vlastní finanční rozhodování.</h3><p>Jako podnikatel jsem byl zvyklý rozhodovat se a nést za svá rozhodnutí odpovědnost. Přesto mě neznalost důležitých podmínek ve smlouvě stála více než 100 000 Kč. Tehdy jsem pochopil, že bez souvislostí člověk snadno podepíše nevýhodnou smlouvu, přehlédne poplatek nebo uvěří nabídce, kterou ještě neumí prověřit.</p><p>Začal jsem proto finance studovat, třídit a převádět do vlastního systému. Postupně jsem se z chaosu vymotal, porozuměl investování a dokázal samostatněji vybírat různé investiční nástroje, promyšleně nakupovat a zhodnocovat vlastní peníze podle pravidel, kterým rozumím. Stejně důležité bylo naučit se odmítnout podezřelé nabídky a podvody, do kterých bych dříve mohl spadnout.</p><p>Restartuj Peníze je uspořádaná verze této cesty. Neukazuje, co máte koupit. Pomáhá vám vytvořit si přehled, rozumět možnostem a poznat, co si potřebujete ověřit před vlastním rozhodnutím.</p></div>
            <div className="founder-detail-panel"><Sparkles className="size-6 text-brand-yellow" /><h3>Zkušenost proměněná v přehlednou mapu</h3><p>Více než 15 let se věnuji vizuální tvorbě, komunikaci a převádění složitých témat do srozumitelné podoby. Proto mají knihy méně odborné mlhy a více příkladů, obrazových souvislostí, kontrolních otázek a jasné struktury.</p><blockquote>„Nechci, abyste kopírovali moje nákupy. Chci, abyste dokázali pochopit vlastní možnosti a lépe posoudit, čemu věřit.“</blockquote></div>
          </div>
