    ...
    <section id="nabidka" className="bg-white px-5 py-22 lg:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="pricing-grid">
          <article className="pricing-card pricing-card-first-aid"><div className="pricing-card-top"><span className="pricing-label"><LifeBuoy /> 2 • konkrétní problémy</span><h3>Finanční první pomoc</h3><p>Šest průvodců pro chvíle, kdy potřebujete vyřešit jeden konkrétní problém a nechcete hledat odpověď v celé knihovně.</p></div><div className="pricing-covers pricing-covers-practical">{practicalBooks.map((book) => <img key={book.image} src={book.image} width="240" height="364" alt="" />)}</div><ul>{['získáte odstup před rozhodnutím pod tlakem', 'srovnáte brokera a zkontrolujete poradce dřív, než podepíšete', 'najdete mezery v rozpočtu, poplatcích i příjmu'].map((item) => <li key={item}><Check />{item}</li>)}</ul><div className="pricing-total"><p>Při nákupu zvlášť 1 194 Kč</p><strong>790 Kč</strong><span>Ušetříte 404 Kč</span></div><AddButton product={products.firstAid} selected={selected(products.firstAid.id)} onAdd={addProduct}>Vybrat praktickou sadu</AddButton></article>

          <article className="pricing-card pricing-card-map"><div className="pricing-card-top"><span className="pricing-label"><BookOpenCheck /> 3 • pevné základy</span><h3>Finanční mapa</h3><p>Dvě hlavní knihy pro člověka, který nechce jen hasit jednotlivé problémy, ale potřebuje pochopit celý systém.</p></div><div className="pricing-covers pricing-covers-main"><img src="/books/financial-restart-hq.jpg" width="1200" height="1818" alt="Finanční (Re)Start" /><img src="/books/investor-dictionary-hq.jpg" width="1200" height="1818" alt="Slovník investora" /></div><ul>{['ujasníte si, co řešit jako první a co může počkat', 'budete rozumět pojmům ve smlouvách a nabídkách', '449 stran názorného systému, ke kterému se můžete vracet'].map((item) => <li key={item}><Check />{item}</li>)}</ul><div className="pricing-total"><p>Při nákupu zvlášť 1 180 Kč</p><strong>990 Kč</strong><span>Ušetříte 190 Kč</span></div><AddButton product={products.map} selected={selected(products.map.id)} onAdd={addProduct}>Vybrat Finanční mapu</AddButton></article>

          <article className="pricing-card pricing-card-featured"><span className="pricing-ribbon">Nejlepší poměr obsahu a ceny</span><div className="pricing-card-top"><span className="pricing-label">4 • kompletní knihovna</span><h3>Celý systém Restartuj Peníze</h3><p>Jedna knihovna pro ochranu před podvody, řešení každodenních problémů i pochopení celého finančního systému.</p></div><div className="pricing-covers pricing-covers-complete"><div className="complete-main-covers"><img src="/books/financial-restart-hq.jpg" width="1200" height="1818" alt="Finanční (Re)Start" /><img src="/books/investor-dictionary-hq.jpg" width="1200" height="1818" alt="Slovník investora" /><img src="/books/mindset.png" width="240" height="364" alt="Mindset úspěšného investora" /></div><div className="complete-practical-covers" aria-label="Šest praktických e-booků">{practicalBooks.map((book) => <img key={book.image} src={book.image} width="240" height="364" alt="" />)}</div><div className="complete-scam-covers" aria-label="Čtyři e-booky o finančních podvodech">{scamBooks.map((book) => <img key={book.id} src={book.image} width="2475" height="3750" alt="" />)}</div></div><ul>{['poznáte časté podvody a podezřelé nabídky', 'vyřešíte konkrétní problém bez hledání v desítkách zdrojů', 'vytvoříte si finanční systém a pravidla pro klidnější rozhodování'].map((item) => <li key={item}><Check />{item}</li>)}</ul><div className="pricing-total"><p>Tři sady samostatně 2 070 Kč</p><strong>1 490 Kč</strong><span>Ušetříte 580 Kč + získáte bonus Mindset</span></div><AddButton product={products.complete} selected={selected(products.complete.id)} onAdd={addProduct} featured>Vybrat kompletní knihovnu</AddButton></article>
        </div>
