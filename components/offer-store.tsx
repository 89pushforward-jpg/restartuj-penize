...
import { ArrowRight, Check, ChevronDown, CircleDollarSign, ShieldAlert, ShieldCheck, ShoppingCart, Trash2 } from 'lucide-react';
    ...
    <section id="nabidka" className="bg-white px-5 py-22 lg:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="pricing-grid">
          <article className="pricing-card"><div className="pricing-card-top"><span className="pricing-label">2 • konkrétní problémy</span><h3>Finanční první pomoc</h3><p>Šest průvodců pro situace, ve kterých lidé nejčastěji ztrácejí peníze, jistotu nebo čas.</p></div><div className="pricing-covers pricing-covers-practical">{practicalBooks.map((book) => <img key={book.image} src={book.image} width="240" height="364" alt="" />)}</div><ul>{['nátlak a emoce při rozhodování', 'broker, rozpočet, příjem a poplatky', 'jak prověřit finančního poradce'].map((item) => <li key={item}><Check />{item}</li>)}</ul><div className="pricing-total"><p>Při nákupu zvlášť 1 194 Kč</p><strong>790 Kč</strong><span>Ušetříte 404 Kč</span></div><AddButton product={products.firstAid} selected={selected(products.firstAid.id)} onAdd={addProduct}>Vybrat praktickou sadu</AddButton></article>

          <article className="pricing-card"><div className="pricing-card-top"><span className="pricing-label">3 • pevné základy</span><h3>Finanční mapa</h3><p>Obě hlavní knihy pro pochopení celku, základních pojmů a investičního jazyka.</p></div><div className="pricing-covers pricing-covers-main"><img src="/books/financial-restart-hq.jpg" width="1200" height="1818" alt="Finanční (Re)Start" /><img src="/books/investor-dictionary-hq.jpg" width="1200" height="1818" alt="Slovník investora" /></div><ul>{['Finanční (Re)Start • 225 stran', 'Slovník investora • 224 stran', '449 stran uspořádaného základu'].map((item) => <li key={item}><Check />{item}</li>)}</ul><div className="pricing-total"><p>Při nákupu zvlášť 1 180 Kč</p><strong>990 Kč</strong><span>Ušetříte 190 Kč</span></div><AddButton product={products.map} selected={selected(products.map.id)} onAdd={addProduct}>Vybrat Finanční mapu</AddButton></article>
